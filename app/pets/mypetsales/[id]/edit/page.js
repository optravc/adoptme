...
export default function EditPetSalePage() {
  ...
  useEffect(() => {
    if (id) {
      fetch(`/api/pets/mypetsales/getpetsale?id=${id}`)
        .then(res => res.json())
        .then(data => {
          setPetDetails({
          ...
          setPreviewUrl(`/upload/petsales/${mapTypeToFolder(data.type)}/${data.image_urlmain}`);
        });
    }
  }, [id]);
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch('/api/pets/mypetsales/updatepetsale', {
      method: 'POST',
      body: formData,
    });
    ...
    if (res.ok) {
      alert('แก้ไขสำเร็จ');
      router.push('/pets/mypetsales'); // กลับไปหน้ารายการ
    } else {
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={6}>
          <Stack spacing={2}>
            ...
            <TextField label="ราคา" name="sales" value={petDetails.sales} onChange={handleInputChange} fullWidth />
