...
export default function AdoptDetailPage() {
  ...
  useEffect(() => {
    const fetchPet = async () => {
      const res = await fetch(`/api/adopt/adoptdetails/${id}`);
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={8}>
              <Box sx={{ borderRadius: 4, overflow: 'hidden', }}>
                <img
                  src={`/upload/petadopt/${mapTypeToFolder(pet.type)}/${pet.image_urlmain}`}
                  alt={pet.pet_name}
                  ...
                  {[pet.image_url2, pet.image_url3, pet.image_url4, pet.image_url5, pet.image_url6]
                    ...
                    .map((img, i) => (
                      <Grid item xs={12} key={i}>
                        ...
                        >
                          <img
                            src={`/upload/petadopt/${mapTypeToFolder(pet.type)}/${img}`}
                            alt={`thumb-${i}`}
                            style={{
                              width: '100%',
                              height: '100%',
