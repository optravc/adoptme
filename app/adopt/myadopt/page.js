...
export default function MyPetAdoptpage() {

  const [MyAdoptpet, setMyAdoptpet] = useState([]);
  ...
  useEffect(() => {
    const fetchSalesPets = async () => {
      if (!user?.id) return;
      const res = await fetch(`/api/adopt/mypetadopt?user_id=${user.id}`);
  ...
  const handleConfirmDelete = async () => {
    try {
      const res = await fetch(`/api/adopt/mypetadopt/delete?id=${deleteId}`, {
        method: 'DELETE',
      });

      if (res.ok) {
        alert('ลบประกาศสำเร็จ');
        setMyAdoptpet(prev => prev.filter(pet => pet.id !== deleteId));
      } else {
  ...
  const handleEditClick = (id) => {
    router.push(`/adopt/myadopt/${id}/edit`);
  };
  return (
    <Box sx={{ p: 4 }}>
      <Typography variant="h4" mb={3}>สัตว์ที่ลงรับเลี้ยงของฉัน</Typography>
          ...
          <Grid item xs={12} sm={6} md={4} key={pet.id}>
            <Card>
              <CardMedia
                component="img"
                height="200"
                image={`/upload/petadopt/${mapTypeToFolder(pet.type)}/${pet.image_urlmain}`}
                alt={pet.pet_name}
              />
              <CardContent>
